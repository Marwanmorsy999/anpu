...
func TestResolveModules_CLIFlagsOverrideConfig(t *testing.T) {
	...
	mc := ResolveModules(models.ProfileStandard, f, true /* --no-nuclei */, false, false, false, false)
	if mc.Nuclei {
...
func TestResolveModules_ProfileDefaultsApplyWithoutConfig(t *testing.T) {
	mc := ResolveModules(models.ProfileSafe, &File{}, false, false, false, false, false)
	if mc.Nuclei {
