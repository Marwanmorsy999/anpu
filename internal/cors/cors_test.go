...
func findingID(fs []models.Finding, id string) *models.Finding {
	for i := range fs { if fs[i].ID == id { return &fs[i] } }
	return nil
}
...
func TestCORS_WildcardWithCredentials(t *testing.T) {
	...
	if f == nil { t.Fatalf("expected wildcard+credentials finding, got %+v", res.Findings) }
	if f.Severity != models.SeverityMedium { t.Fatalf("expected medium severity, got %s", f.Severity) }
}
...
func TestCORS_WildcardOnly_LowSeverity(t *testing.T) {
	...
	if f == nil { t.Fatalf("expected wildcard finding, got %+v", res.Findings) }
	if f.Severity != models.SeverityLow { t.Fatalf("expected low severity, got %s", f.Severity) }
}
...
func TestCORS_ReflectsArbitraryOriginWithCredentials(t *testing.T) {
	...
	if f == nil { t.Fatalf("expected reflection finding, got %+v", res.Findings) }
	if f.Severity != models.SeverityHigh { t.Fatalf("expected high severity, got %s", f.Severity) }
}

func TestCORS_Name(t *testing.T) {
	if got := New(anpuhttp.NewClientWithLocalNetworkAllowed(true)).Name(); got != "cors" { t.Fatalf("got %q", got) }
}
