...
func (c *Crawler) Discover(ctx context.Context, startURL string) ([]models.Endpoint, []string, error) {
	...
	for len(queue) > 0 && len(visited) < c.limits.MaxPages {
		...
		if !isHTML(resp.Header.Get("Content-Type")) || current.depth >= c.limits.MaxDepth {
			continue
		}
		...
		for _, link := range links {
			...
			if current.depth+1 > c.limits.MaxDepth || !shouldCrawl(resolved) || queued[resolved] || visited[resolved] {
				continue
			}
