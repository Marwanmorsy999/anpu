...
func TestNuclei_Name(t *testing.T) {
	if got := NewNucleiScanner().Name(); got != "nuclei" {
		t.Fatalf("got %q", got)
	}
}
...
func TestNuclei_Run_NotInstalled_ReturnsWarning(t *testing.T) {
	...
	if len(res.Warnings) == 0 || len(res.Findings) != 0 {
		t.Fatalf("expected warning/no findings, got warnings=%v findings=%v", res.Warnings, res.Findings)
	}
}
...
func fakeNucleiScript(t *testing.T) string {
	...
	if runtime.GOOS == "windows" {
		t.Skip("POSIX fake nuclei script is not portable to Windows")
	}
...
func TestNucleiTemplateTagsForProfile(t *testing.T) {
	...
	for _, tc := range cases {
		if got := strings.Join(nucleiTemplateTagsForProfile(tc.profile), " "); !strings.Contains(got, tc.want) {
			t.Errorf("profile %s: got %q", tc.profile, got)
		}
	}
}
...
func TestConvertNucleiFinding_InvalidSeverityDefaultsToInfo(t *testing.T) {
	...
	if got := convertNucleiFinding(nl, "http://example.com").Severity; got != models.SeverityInfo {
		t.Fatalf("got %s", got)
	}
}
