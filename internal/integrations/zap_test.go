...
func TestZap_Name(t *testing.T) {
	if got := NewZapScanner().Name(); got != "zap" { t.Fatalf("got %q", got) }
}

func TestZap_AlwaysUnavailable(t *testing.T) {
	if NewZapScanner().Available(context.Background()) { t.Fatal("expected ZAP to be unavailable in MVP") }
}
...
func TestZap_Run_ReturnsWarningNotError(t *testing.T) {
	...
	if len(res.Warnings) != 1 { t.Fatalf("expected one warning, got %v", res.Warnings) }
}
