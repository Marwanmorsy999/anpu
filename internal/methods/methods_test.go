...
func TestMethods_AdvertisedPUT(t *testing.T) {
	...
	if !hasMethodFinding(res.Findings, "methods-advertised-put") {
		t.Fatalf("expected PUT finding, got %+v", res.Findings)
	}
}
...
func TestMethods_TraceEchoesMarker(t *testing.T) {
	...
	if !hasMethodFinding(res.Findings, "methods-trace-enabled") {
		t.Fatalf("expected TRACE finding, got %+v", res.Findings)
	}
}
...
func TestMethods_TraceRejected_NoFinding(t *testing.T) {
	...
	if hasMethodFinding(res.Findings, "methods-trace-enabled") {
		t.Fatalf("unexpected TRACE finding: %+v", res.Findings)
	}
}
...
func TestParseAllow(t *testing.T) {
	...
	if got["TRACE"] {
		t.Error("TRACE should not be present")
	}
}

func TestMethods_Name(t *testing.T) {
	if got := New(anpuhttp.NewClientWithLocalNetworkAllowed(true)).Name(); got != "methods" {
		t.Fatalf("got %q", got)
	}
}
