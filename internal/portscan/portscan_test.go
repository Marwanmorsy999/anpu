...
func TestPortScan_DetectsInjectedOpenPort(t *testing.T) {
	...
	s.dial = func(ctx context.Context, network, address string) (net.Conn, error) {
		if strings.HasSuffix(address, ":8888") {
			return &fakeConn{}, nil
		}
	...
	for _, f := range res.Findings {
		if f.ID == "portscan-open-8888" {
			found = true
			break
		}
	}
	if !found {
		t.Fatalf("expected open 8888 finding, got %+v", res.Findings)
	}
}
...
func TestPortScan_SanityProbeSuppressesResults(t *testing.T) {
	s := New()
	s.dial = func(ctx context.Context, network, address string) (net.Conn, error) {
		if strings.HasSuffix(address, ":1") || strings.HasSuffix(address, ":9") {
			return &fakeConn{}, nil
		}
		return nil, fmt.Errorf("closed")
	}
	...
	if len(res.Findings) != 0 {
		t.Fatalf("expected suppressed findings, got %+v", res.Findings)
	}
