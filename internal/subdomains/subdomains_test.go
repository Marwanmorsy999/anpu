...
func TestSubdomains_Name(t *testing.T) {
	if got := New().Name(); got != "subdomains" { t.Fatalf("got %q", got) }
}

func TestSubdomains_Available(t *testing.T) {
	if !New().Available(context.Background()) { t.Fatal("expected available") }
}
...
func TestSubdomains_BareHost_NoFindings(t *testing.T) {
	...
	if len(res.Findings) != 0 || len(res.Warnings) != 0 { t.Fatalf("expected empty result, got findings=%+v warnings=%v", res.Findings, res.Warnings) }
}
...
func TestQueryCTLogs_ParsesSubdomains(t *testing.T) {
	...
	if warn != "" { t.Fatalf("unexpected warning: %s", warn) }
	joined := strings.Join(names, ",")
	for _, want := range []string{"www.example.com", "api.example.com", "staging.example.com", "*.example.com"} {
		if !strings.Contains(joined, want) { t.Fatalf("missing %q in %v", want, names) }
	}
}
...
func TestQueryCTLogs_NonOKStatus_Warns(t *testing.T) {
	...
	s.client = &http.Client{Transport: &redirectTransport{targetURL: srv.URL}, Timeout: 5 * time.Second}
	names, warn := s.queryCTLogs(context.Background(), "example.com")
	if warn == "" || len(names) != 0 { t.Fatalf("expected warning and no names, got names=%v warning=%q", names, warn) }
}

func TestDNSBruteSuffix(t *testing.T) {
	if got := dnsBruteSuffix(models.ProfileSafe); got != "" { t.Fatalf("safe suffix=%q", got) }
	if got := dnsBruteSuffix(models.ProfileDeep); !strings.Contains(got, "brute") { t.Fatalf("deep suffix=%q", got) }
}

func TestExtraSuffix(t *testing.T) {
	if got := extraSuffix(0); got != "" { t.Fatalf("zero suffix=%q", got) }
	if got := extraSuffix(5); !strings.Contains(got, "5 more") { t.Fatalf("five suffix=%q", got) }
}
